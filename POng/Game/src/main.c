...
void ballCollision(object *b) {
	if(b->x < 0) b->vel_x = fabs(b->vel_x);
	if(b->x > WINDOW_WIDTH - BALL_SIZE) b->vel_x = -fabs(b->vel_x);
}
...
void gg(object *ball) {
	if(ball->y < MARGIN) updateScore(1, 1);
	if(ball->y > WINDOW_HEIGHT - MARGIN) updateScore(2, 1);
}
